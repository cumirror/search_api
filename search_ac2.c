...
struct ac_search_data {
    ACSM_STRUCT2 *acsm;
};

static int ac_init(struct Search *s_obj)
{
    struct ac_search_data *sd = (struct ac_search_data *)(s_obj->priv);
...
static int ac_add(struct Search *s_obj, char *pattern, int len, int id)
{
    struct ac_search_data *sd = (struct ac_search_data *)(s_obj->priv);

    acsmAddPattern2(sd->acsm, (unsigned char*)pattern, len, 1/*no case*/,
            0, 0, 0, (void*)NULL, id);

    return 0;
}

static int ac_build(struct Search *s_obj)
{
    struct ac_search_data *sd = (struct ac_search_data *)(s_obj->priv);

    acsmCompile2(sd->acsm, NULL, NULL);
    return 0;
}

static int ac_search(struct Search *s_obj, char *text, int n)
{
    struct ac_search_data *sd = (struct ac_search_data *)(s_obj->priv);
    int cur = 0;

    acsmSearch2(sd->acsm, (unsigned char *)text, n, MatchFound, (void *)NULL, &cur);

    return 0;
}

static int ac_dump(struct Search *s_obj)
{
    struct ac_search_data *sd = (struct ac_search_data *)(s_obj->priv);

    acsmPrintInfo2(sd->acsm);
    return 0;
}

static int ac_destroy(struct Search *s_obj)
{
    struct ac_search_data *sd = (struct ac_search_data *)(s_obj->priv);
    acsmFree2(sd->acsm);
    free(s_obj);
...
static struct Search_ops ac_search_ops = {
    .name = "ac",
    .priv_size = sizeof(struct ac_search_data),
    .init = ac_init,
    .add = ac_add,
    .build = ac_build,
    .search = ac_search,
...
int ac_module_init()
{
    return register_search_ops(&ac_search_ops);
}
